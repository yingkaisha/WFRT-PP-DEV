...
def ax_decorate(ax, font_text, left_flag, bottom_flag, bottom_spline=False):
...
def ax_decorate_box(ax, font_text):
    ax.xaxis.set_tick_params(labelsize=font_text)
    ax.yaxis.set_tick_params(labelsize=font_text)
    [j.set_linewidth(2.5) for j in ax.spines.values()]
...
def lg_decorate(ax, font_text, loc=(0.9, 0.9)):
    LG = ax.legend(bbox_to_anchor=loc, ncol=1, prop={'size':font_text});
...
def precip_map_single(CNN_Px, lon, lat, lead_hrs, accum_hrs, dt_utc_now, 
...
def xcolor(key):
    '''
    A list commonly used colors by names.
    '''
